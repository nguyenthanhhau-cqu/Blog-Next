...
function Gallery({images,onSelect, uploading=false,selectedImage=""}:GalleryProps) {
 return (
  <div className='flex flex-wrap'>
      {uploading && <div className='basis-1/4 p-2 aspect-square flex flex-col items-center justify-center
      bg-secondary-light text-primary-dark rounded animate-pulse'>
            <BsCardImage size={60}/>
          <p>Uploading</p>
      </div>}
      {images1.map((item,index)=> {
          return <div key={index}  className='basis-1/4 p-2'>
              <Image alt='image gallery' onClick={()=> onSelect ? onSelect(item.src) : ''} src={item.src} selected={selectedImage === item.src}/>
          </div>
      })}
  </div>
 );
